...
public class player_green extends player {
    ...
    public void Update(long GameTime){
        ...
        if(GameTime > mFrameTimer + 2000){
            ...
            m_pmslist.add(new Missile(AppManager.getInstance().getBitmap(R.drawable.ice_bolt), m_x + 350, m_y + 135, 15, 0));
            m_pmslist.get(m_pmslist.size() -1).missiletype = 0;
            int j = 0;
            for (int i = 0; i < 10; i++) {
                j = random.nextInt(2);
                if(j == 0) j = -1;
                m_pmslist.add(new Missile(AppManager.getInstance().getBitmap(R.drawable.ice_1), m_x + 350, m_y + 135, random.nextInt(14) + 1, j * (random.nextInt(14) + 1)));
                m_pmslist.get(m_pmslist.size() -1).missiletype = 1;
            }
        }
